# Postman Workspace Provisioning Tools — JavaScript

Comprehensive tooling for automated Postman partner workspace provisioning and management. This is a standalone JavaScript module that can be copied into any Node.js project.
...
Copy `postmanService.js` into your project and install the dependency:
...
```bash
cd dev-portal/javascript/script
npm install
```
...
const results = await resetWorkspace(
  ...
  { includeSpecs: true, includeMocks: true, includeEnvironments: false, includeCollections: false }
);
```
...
```javascript
import { addWorkspaceAdmin, invitePartner, inviteMultiplePartners } from './postmanService.js';
...
```javascript
import { validateApiKey, getWorkspaceSummary, getConfigurationStatus } from './postmanService.js';
...
All async functions accept a progress callback:

```javascript
(progress) => {
  progress.phase      // 'validation' | 'workspace' | 'collections' | 'mocks' | 'environments' | 'mockEnv' | 'specs' | 'admins' | 'partners' | 'complete' | 'error'
...
```javascript
try {
  const results = await provisionWorkspace({ ... }, (progress) => console.log(progress));
