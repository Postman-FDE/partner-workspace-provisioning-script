...
```java
postmanService.updateWorkspaceAssets("source-workspace-id", "target-workspace-id")
    .subscribe(result -> {
        System.out.println("New collections: " + result.get("newCollections"));
        System.out.println("New specs: " + result.get("newSpecs"));
        System.out.println("New environments: " + result.get("newEnvironments"));
    });
```
...
```java
Consumer<Map<String, Object>> onProgress = progress -> {
    String phase = (String) progress.get("phase");       // validation, workspace, collections, mocks, environments, mockEnv, collectionVars, specs, admins, partners, complete, error
